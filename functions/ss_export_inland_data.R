...
# edits made by Nicole Torrie to adjust code for inland datasets (function adapted from
# sensorstrings code)



ss_export_data_files <- function(
    dat,
#    county = "",
    output_path = NULL,
    export_csv = TRUE,
  ...
  file_name <- paste(today_date, "inland_water_quality_dataset", sep = "_")
  ...
  if(isTRUE(export_csv)) {
    ...
    if(is.null(output_path)){
      ...
      # output file name
      output_csv <- paste0(
        "R:/data_branches/inland_water_quality/open_data/", file_name, ".csv")
  ...
  if(isTRUE(export_rds)) {
    ...
    if(is.null(output_path)){
      ...
      output_rds <- paste0(
        "R:/data_branches/inland_water_quality/processed_data/assembled_data/",
        file_name, ".rds")
